...
const HomeSingle = ({ homepage }) => {
  ...
  const submit = async () => {
    if (htno.length != 10) {
      ...
      try
      {
        const response = await axios.get(url+'/api/academicresult?htno=' + htno, { mode: 'cors' });
  ...
  return (
    <>
      <div method="get" className="mx-[0.25%] border-[3px] rounded-md border-primary border-solid">
        <center>
          <br />
          ...
          <br />
          <br />
          <button type="submit" onClick={submit} className="w-[70px] text-white	bg-green-700 rounded text-[60%] hover:bg-yellow-400 py-[0.15em] px-[1.2em] sm:w-[100px] sm:text-[100%]" >
            Results
          </button>
          <br />
          <br />
